...
const meta: Meta<typeof PktsTabs> = {
  ...
  render: (args) => {
    const tabs = document.createElement(PktsTabs.tagName) as InstanceType<
      typeof PktsTabs
    >;

    if (args.tabs && Array.isArray(args.tabs)) {
      args.tabs.forEach(
        (tab: { label: string; active?: boolean }, index: number) => {
          const li = document.createElement("li");
          li.className =
            tab.active || index === 0 ? "pkts-tab pkts-active" : "pkts-tab";
