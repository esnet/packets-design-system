...
export interface ESDataTableProps {
...
export class ESDataTable extends HTMLElement implements ESDataTableProps {
  static tagName = "es-data-table";
  ...
  protected render(): void {
    if (!this._columns || !this._data) {
      this.innerHTML = `<table class="es-data-table"></table>`;
      return;
    }
        ...
        return `
          <th ${widthAttr}>
            <div class="labelWrapper" ${isSortable ? `data-sort-key="${col.key}"` : ""} ${isSortable ? `style="cursor: pointer;"` : ""}>
              ${col.label}
              ${isSortable && sortDirection !== "NONE" ? `<span class="es-data-table-sort-icon">${this._renderSortIcon(sortDirection)}</span>` : ""}
            </div>
          </th>
        `;
      })
...
customElements.define(ESDataTable.tagName, ESDataTable);
