export const monthNames = [
  "Jan",
  "Feb",
  "Mar",
  "Apr",
  "May",
  "Jun",
  "Jul",
  "Aug",
  "Sep",
  "Oct",
  "Nov",
  "Dec",
];

export const weekdayNames = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"];

export function getMonthName(date: Date): string {
  return monthNames[date.getMonth()] ?? "";
}

export function getWeekdayName(date: Date): string {
  return weekdayNames[date.getDay()] ?? "";
}

/**
 * Given two dates, return them in order from earliest to latest (swapping them).
 */
export function orderDates(first: Date, second: Date): [Date, Date] {
  const _first = new Date(first);
  const _second = new Date(second);
  if (first.getTime() > second.getTime()) {
    return [_second, _first];
  }
  return [_first, _second];
}

/**
 * To have a proper 6 row x 7 column grid of dates, there needs to be dates padding the current date's month,
 * which are taken from the previous and next month.
 * This function returns a flattened array of dates that fill out that grid.
 */
export function flattenedDateGrid(date: Date): Date[] {
  const dates = [];
  const firstDay = new Date(date.getFullYear(), date.getMonth(), 1);
  firstDay.setHours(0, 0, 0, 0);
  const daysInCurrentMonth = new Date(
    date.getFullYear(),
    date.getMonth() + 1,
    0
  ).getDate();

  // days from previous month
  dates.push(
    ...Array.from(
      { length: firstDay.getDay() },
      (_, i) =>
        new Date(
          date.getFullYear(),
          date.getMonth(),
          -1 * (firstDay.getDay() - i)
        )
    )
  );

  // days from current month
  dates.push(
    ...Array.from(
      { length: daysInCurrentMonth },
      (_, i) => new Date(date.getFullYear(), date.getMonth(), i + 1)
    )
  );

  // dates from next month
  dates.push(
    ...Array.from(
      { length: 42 - dates.length }, // 6 rows x 7 columns = 42 total cells
      (_, i) => new Date(date.getFullYear(), date.getMonth() + 1, i + 1)
    )
  );

  return dates;
}
