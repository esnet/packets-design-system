...
export class PktsButton extends SlottedComponent implements PktsButtonProps {
  static tagName = "pkts-button";
  ...
  set variant(
    v: "primary" | "secondary" | "branded" | "tertiary" | "destructive",
  ) {
  ...
  get size(): "medium" | "xxlarge" {
    return (this.getAttribute("size") as any) ?? "medium";
  }
  set size(v: "medium" | "xxlarge") {
  ...
  set fill(v: boolean) {
    v ? this.setAttribute("fill", "") : this.removeAttribute("fill");
  }
  ...
  get as(): "button" | "a" {
    return (this.getAttribute("as") as any) ?? "button";
  }
  set as(v: "button" | "a") {
  ...
  protected _renderInitial(): void {
    ...
    this.innerHTML = `
            <${tag} type='${this.type}' class="pkts-button">
                <slot></slot>
            </${tag}>
        `;
  ...
  protected render(): void {
    ...
    const fill = this.fill ? "pkts-fill" : "pkts-nofill";
    const size = this.size ? `pkts-${this.size}` : "";
...
customElements.define(PktsButton.tagName, PktsButton);
