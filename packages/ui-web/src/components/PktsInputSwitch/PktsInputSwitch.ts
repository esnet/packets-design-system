...
export class PktsInputSwitch
  extends HTMLElement
  implements PktsInputSwitchProps
{
  static tagName = "pkts-input-switch";
  ...
  set error(v: boolean) {
    v ? this.setAttribute("error", "") : this.removeAttribute("error");
  }

  get hideIcon(): boolean {
    return this.hasAttribute("hide-icon");
  }
  set hideIcon(v: boolean) {
    v ? this.setAttribute("hide-icon", "") : this.removeAttribute("hide-icon");
  }
  ...
  protected render(): void {
    ...
    this._switchBtn.innerHTML = !this.hideIcon
      ? `<${PktsIcon.tagName} name="${this.checked ? "check" : "X"}"></${PktsIcon.tagName}>`
      : "";
  }
}

customElements.define(PktsInputSwitch.tagName, PktsInputSwitch);
