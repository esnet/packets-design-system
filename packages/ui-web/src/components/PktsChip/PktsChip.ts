...
export class PktsChip extends SlottedComponent implements PktsChipProps {
  static tagName = "pkts-chip";
  ...
  protected _renderInitial(): void {
    this.innerHTML = `
      <button type="button" class="pkts-chip">
        <slot></slot>
      </button>
    `;
  ...
  protected render(): void {
    ...
    if (this.variant === "outline") {
      classes.push("pkts-outline");
    }

    if (this.rounded) {
      classes.push("pkts-rounded");
    }

    if (this.disabled) {
      classes.push("pkts-disabled");
    }
...
customElements.define(PktsChip.tagName, PktsChip);
