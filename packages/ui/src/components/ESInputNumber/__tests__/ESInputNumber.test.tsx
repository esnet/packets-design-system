...
import ESInputNumber from "../ESInputNumber";
...
test.describe("ESInputNumber", () => {
  const {
    ...
    themes,
    actionStates,
  }: ComponentTestTableType<ESInputNumberProps> = {
    testTable: [
      {
        name: "primary",
  ...
  test("can change value with typing", async ({ mount }) => {
    const component = await mount(<ESInputNumber />);
  ...
  test("does not exceed max value of 2", async ({ mount }) => {
    const component = await mount(<ESInputNumber max={2} />);
    const input = component.locator("input");
    const addButton = component.locator("svg").first();
...
//       <ESInputNumber value="Invalid" variant="branded" error />
//     );
//     await expect(component).toHaveScreenshot();
//   });
//   test("placeholder shows", async ({ mount }) => {
//     const component = await mount(
//       <ESInputNumber placeholder="Enter text here" />
//     );
//     const input = component.locator("input");
