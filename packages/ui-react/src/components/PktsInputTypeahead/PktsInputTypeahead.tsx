...
import { PktsInputTypeaheadProps } from "./PktsInputTypeahead.types";
 ...
 * For single select with a simpler interface, see `PktsInputSelect`.
 *
 ...
 *
 * This component differs from the HTML select element (and from `PktsInputSelect`) because it manages the value as a list of values,
 * rather than a singular one. For reference, `PktsInputSelect`'s value is `string`, while this component is `string[]`.
 *
 * @param {PktsInputTypeaheadProps} props
 * @returns {React.ReactElement}
 */
export function PktsInputTypeahead({
  variant = "primary",
  error = false,
  disabled = false,
  className,
  ...
  loading = false,
  children,
}: PktsInputTypeaheadProps) {
  /*
   * PktsInputTypeahead is composed of three parts (ordered by visual top to bottom appearance)
...
PktsInputTypeahead.displayName = "PktsInputTypeahead";

export default PktsInputTypeahead;
